...
void TriggerEstimate::Loop(){
  ...
  fChain->SetBranchStatus("HLT_BIT_HLT_QuadJet45_DoubleBTagCSV_p087_v",1);
   //Di90Di30 block
  fChain->SetBranchStatus("ntrgObjects_hltL1sQuadJetC50IorQuadJetC60IorHTT280IorHTT300IorHTT320IorTripleJet846848VBFIorTripleJet887256VBFIorTripleJet927664VBF",1);
  ...
  TFile *w = TFile::Open("/fdata/hepx/store/user/delgado_andrea/94X/updatedNtuplesJune19/weightHistos_Quad45_H_v2.root");

  TEfficiency *l1 = (TEfficiency*) w->Get("L1Quad45_caloT");
  TEfficiency *t1 = (TEfficiency*) w->Get("Quad45Calo_caloT");
  TEfficiency *t2 = (TEfficiency*) w->Get("Quad45LeadCSV3_min_PF");
  TEfficiency *t3 = (TEfficiency*) w->Get("Quad45PF_PfT");  

  Double_t xbins[32] = {100., 120., 140., 160., 180., 200., 220., 240., 260., 280., 300., 320., 340., 360., 380., 400., 425., 450., 475., 500., 525., 550., 600., 650., 700., 750., 800., 850., 900., 1000., 1100., 1200.};
  ...
  if(!applyWeights){                           
    ...
    histograms["CutflowQ45"] = new TEfficiency("CutflowDi90Di30","IsoMu24, GlobalMuon pT>54, 4pF jets>30, L1, Calo4, PF4, Calo2, PF2, CSV;category;acceptance",7, -0.5,6.5);
    
    histograms["L1Quad45_caloT"] = new TEfficiency("L1Quad45_caloT","L1 turnon Quad45 calo trg objects;#Sigma p_{T} 0-3 [GeV];efficiency",100,100.,600.); 
    histograms["L1Quad45_PfT"] = new TEfficiency("L1Quad45_PfT","L1 turnon Quad45 PF trg objects;#Sigma p_{T} 0-3 [GeV];efficiency",100,100.,600.);   
    histograms["L1Quad45_PF"] = new TEfficiency("L1Quad45_PF","L1 turnon Quad45 PF;#Sigma p_{T} 0-3 [GeV];efficiency",100,100.,600.);

    histograms["Quad45Calo_caloT"] = new TEfficiency("Quad45Calo_caloT","HLT Calo45 turnon calo trg objects;p_{T} 4 [GeV];efficiency",60,20.,140.);  
    histograms["Quad45Calo_PfT"] = new TEfficiency("Quad45Calo_PfT","HLT Calo45 turnon PF trg objects;p_{T} 4 [GeV];efficiency",60,20.,140.);   
    histograms["Quad45Calo_PF"] = new TEfficiency("Quad45Calo_PF","HLT Calo45 turnon unmatched PF;p_{T} 4 [GeV];efficiency",60,20.,140.);     

    histograms["Quad45PF_PfT"] = new TEfficiency("Quad45PF_PfT","HLT PF45 turnon PF trg objects;p_{T} 4 [GeV];efficiency",60,20.,140.);     
    histograms["Quad45PF_PF"] = new TEfficiency("Quad45PF_PF","HLT PF45 turnon unmatched PF;p_{T} 4 [GeV];efficiency",60,20.,140.);  

    histograms["Quad45LeadCSV3_max_mTrgPF"] = new TEfficiency("Quad45LeadCSV3_max_mTrgPF","HLT leading CSV turnon trg match to PF;max CSV discriminator value;efficiency",100.,0.,1.);  
    histograms["Quad45LeadCSV3_max_PF"] = new TEfficiency("Quad45LeadCSV3_max_PF","HLT leading CSV turnon unmatched PF;max CSV discriminator value;efficiency",100.,0.,1.);   
    histograms["Quad45LeadCSV3_min_mTrgPF"] = new TEfficiency("Quad45LeadCSV3_min_mTrgPF","HLT leading CSV turnon trg match to PF;min_3 CSV discriminator value;efficiency",100.,0.,1.);
    histograms["Quad45LeadCSV3_min_PF"] = new TEfficiency("Quad45LeadCSV3_min_PF","HLT leading CSV turnon unmatched PF;min_3 CSV discriminator value;efficiency",100.,0.,1.);   
    histograms["Quad45LeadCSV3_min2_mTrgPF"] = new TEfficiency("Quad45LeadCSV3_min2_mTrgPF","HLT leading CSV turnon trg match to PF;min_2 CSV discriminator value;efficiency",100.,0.,1.);
    histograms["Quad45LeadCSV3_min2_PF"] = new TEfficiency("Quad45LeadCSV3_min2_PF","HLT leading CSV turnon unmatched PF;min_2 CSV discriminator value;efficiency",100.,0.,1.);   
  }
  else{
    ...
    histogramsVal["trigmjjc1"] = new TH1D("trigmjjc1", "Triggered Events vs mjj in 2T", 31, xbins);
    histogramsVal["trigmjjc2"] = new TH1D("trigmjjc2", "Triggered Events vs mjj in 2T!(3T||4T)", 31, xbins);
    histogramsVal["trigmjjc3"] = new TH1D("trigmjjc3", "Triggered Events vs mjj in 3T!4T", 31, xbins);
    histogramsVal["trigmjjc4"] = new TH1D("trigmjjc4", "Triggered Events vs mjj in 4T ", 31, xbins);

    histogramsVal["weightmjjc1"] = new TH1D("weightmjjc1", "Weighted Events vs mjj in 2T", 31, xbins);
    histogramsVal["weightmjjc2"] = new TH1D("weightmjjc2", "Weighted Events vs mjj in 2T!(3T||4T)", 31, xbins);
    histogramsVal["weightmjjc3"] = new TH1D("weightmjjc3", "Weighted Events vs mjj in 3T!4T", 31, xbins);
    histogramsVal["weightmjjc4"] = new TH1D("weightmjjc4", "Weighted Events vs mjj in 4T ", 31, xbins); 
...
//    if(jentry%10==0) cout<<jentry<<"/"<<nentries2<<endl;
    ...
    if(!applyWeights) histograms["CutflowQ45"]->Fill(HLT_BIT_HLT_IsoMu24_v,0);
    ...
    if(!applyWeights) histograms["CutflowQ45"]->Fill(passMuon,1);
    ...
    //----------------------------jet selection test
    if(!applyWeights) histograms["CutflowQ45"]->Fill(PFjets.size()>=4,2);
    ...
    if(applyWeights){
      ...
      weights.push_back(weight(t2, MinBtag));
      weights.push_back(weight(t3, trgObjects_pfJets_pt[min(ntrgObjects_pfJets-1,3)]));
      ...
      if(HLT_BIT_HLT_QuadJet45_TripleBTagCSV_p087_v){
    ...
    }//end of applyWeights loop

    else{
      histograms["CutflowQ45"]->Fill(ntrgObjects_hltL1sQuadJetC50IorQuadJetC60IorHTT280IorHTT300IorHTT320IorTripleJet846848VBFIorTripleJet887256VBFIorTripleJet927664VBF,3);
      histograms["L1Quad45_caloT"]->Fill(ntrgObjects_hltL1sQuadJetC50IorQuadJetC60IorHTT280IorHTT300IorHTT320IorTripleJet846848VBFIorTripleJet887256VBFIorTripleJet927664VBF>=1, SumCalo);
      
      SumCalo=0.;
      for(unsigned c=0; c<min(ntrgObjects_pfJets,4); ++c) SumCalo+=trgObjects_pfJets_pt[c];
      histograms["L1Quad45_PfT"]->Fill(ntrgObjects_hltL1sQuadJetC50IorQuadJetC60IorHTT280IorHTT300IorHTT320IorTripleJet846848VBFIorTripleJet887256VBFIorTripleJet927664VBF>=1, SumCalo); 

      SumCalo=0.;
      for(unsigned c=0; c<min((int)PFjets.size(),4); ++c) SumCalo+=PFjets[c].Pt();
      histograms["L1Quad45_PF"]->Fill(ntrgObjects_hltL1sQuadJetC50IorQuadJetC60IorHTT280IorHTT300IorHTT320IorTripleJet846848VBFIorTripleJet887256VBFIorTripleJet927664VBF>=1, SumCalo);
      
      if(verbose) cout<<"L1 histos filled"<<endl;
      
      if(ntrgObjects_hltL1sQuadJetC50IorQuadJetC60IorHTT280IorHTT300IorHTT320IorTripleJet846848VBFIorTripleJet887256VBFIorTripleJet927664VBF>=1){
      //Quad45Calo
        histograms["Quad45Calo_caloT"]->Fill(ntrgObjects_hltQuadCentralJet45>=4,trgObjects_caloJets_pt[min(ntrgObjects_caloJets-1,3)]);
        histograms["Quad45Calo_PfT"]->Fill(ntrgObjects_hltQuadCentralJet45>=4,trgObjects_pfJets_pt[min(ntrgObjects_pfJets-1,3)]);
        histograms["Quad45Calo_PF"]->Fill(ntrgObjects_hltQuadCentralJet45>=4,PFjets[3].Pt());
        if(verbose) cout<<"Quad45 histos filled"<<endl;
        
        if(ntrgObjects_hltQuadCentralJet45>=4){
          if(CSVvals.size()){
            if(verbose) cout<<"CSVvalssize: " << CSVvals.size()<<endl;              
              histograms["Quad45LeadCSV3_max_mTrgPF"]->Fill(ntrgObjects_hltBTagCaloCSVp087Triple>=3,CSVvals[0]);
              histograms["Quad45LeadCSV3_min_mTrgPF"]->Fill(ntrgObjects_hltBTagCaloCSVp087Triple>=3,CSVvals[min(2,(int)CSVvals.size()-1)]);
              histograms["Quad45LeadCSV3_min2_mTrgPF"]->Fill(ntrgObjects_hltBTagCaloCSVp087Triple>=3,CSVvals[min(1,(int)CSVvals.size()-1)]);
          }
      
          histograms["Quad45LeadCSV3_max_PF"]->Fill(ntrgObjects_hltBTagCaloCSVp087Triple>=3,MaxBtag);
          histograms["Quad45LeadCSV3_min_PF"]->Fill(ntrgObjects_hltBTagCaloCSVp087Triple>=3,MinBtag);
          histograms["Quad45LeadCSV3_min2_PF"]->Fill(ntrgObjects_hltBTagCaloCSVp087Triple>=3,PreMinBtag);
          if(verbose) cout<<" CSV histos filled"<<endl;
            
          if(ntrgObjects_hltBTagCaloCSVp087Triple>=3){
            //Quad45PF
            histograms["Quad45PF_PfT"]->Fill(ntrgObjects_hltQuadPFCentralJetLooseID45>=4,trgObjects_pfJets_pt[min(ntrgObjects_pfJets-1,3)]);
            histograms["Quad45PF_PF"]->Fill(ntrgObjects_hltQuadPFCentralJetLooseID45>=4,PFjets[3].Pt());
            if(verbose) cout<<"Quad45PF histos filled"<<endl;
          }
        }
      }
  ...
  if(applyWeights){
    savefile = new TFile("triggerClosure_Quad45_"+name+".root","RECREATE");
    for(std::map<string,TH1*>::iterator it=histogramsVal.begin(); it!=histogramsVal.end(); ++it){
      histogramsVal[it->first]->Write(); 
    }
  }   
  else{
    savefile = new TFile("weightHistos_Quad45_"+name+"_v2.root","RECREATE");
    histograms["CutflowQ45"]->Write();
    histograms["L1Quad45_caloT"]->Write(); 
    histograms["L1Quad45_PfT"]->Write(); 
    histograms["L1Quad45_PF"]->Write(); 
    histograms["Quad45Calo_caloT"]->Write();
    histograms["Quad45Calo_PfT"]->Write(); 
    histograms["Quad45Calo_PF"]->Write();
    histograms["Quad45PF_PfT"]->Write();
    histograms["Quad45PF_PF"]->Write();
    histograms["Quad45LeadCSV3_max_mTrgPF"]->Write();
    histograms["Quad45LeadCSV3_max_PF"]->Write();
    histograms["Quad45LeadCSV3_min_mTrgPF"]->Write();
    histograms["Quad45LeadCSV3_min_PF"]->Write();
    histograms["Quad450LeadCSV3_min2_mTrgPF"]->Write();
    histograms["Quad45LeadCSV3_min2_PF"]->Write(); 
  }
